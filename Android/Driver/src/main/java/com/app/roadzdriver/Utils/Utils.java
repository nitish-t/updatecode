...
public abstract class Utils {
    ...
    public static void showOkDialogWithActionWithCancel(final Activity activity, final Runnable run, final Runnable runCancel, String msg, int dialogType) {
        if (activity == null) return;
        new SweetAlertDialog(activity, dialogType)
                .setTitleText("")
    ...
    public static boolean Valed(Context context, EditText et_fild, EditText et_fild2, Type type) {
        switch (type) {
            ...
            case pass:
                ...
                break;
            case phone:
                if (TextUtils.isEmpty(et_fild.getText().toString().trim()) || et_fild.getText().length() != 14) {
